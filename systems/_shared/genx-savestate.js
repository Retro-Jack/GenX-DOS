...
(function () {
  ...
  function ready() { var e = emu(); return e && e.gameManager && typeof e.gameManager.getState === 'function'; }

  // chunked to avoid call-stack overflow on large states
  function toB64(u8) { var s = '', C = 0x8000; for (var i = 0; i < u8.length; i += C) s += String.fromCharCode.apply(null, u8.subarray(i, i + C)); return btoa(s); }
  ...
  function refocus() {
    try {
      var e = emu();
      var el = (e && e.elements && e.elements.parent) || (e && e.canvas) || document.querySelector('#game canvas, canvas');
  ...
  function getStateBytes() {
    var e = emu();
    if (!e.gameManager._gxInfo) e.gameManager._gxInfo = e.Module.cwrap("save_state_info", "string", []);
    var parts = e.gameManager._gxInfo().split("|"); // "length|pointer|status"
    if (parts[2] !== "1") throw new Error(parts[0] || "state unsupported");
    var len = parseInt(parts[0], 10), ptr = parseInt(parts[1], 10);
  ...
  function setStateBytes(bytes) {
    var e = emu(), FS = e.Module.FS;
    try { FS.unlink("game.state"); } catch (x) {}
    FS.writeFile("/game.state", bytes);
    if (!e.gameManager._gxLoad) e.gameManager._gxLoad = e.Module.cwrap("load_state", "number", ["string", "number"]);
    var r = e.gameManager._gxLoad("game.state", 0);
    setTimeout(function () { try { FS.unlink("game.state"); } catch (x) {} }, 4000);
    return r;
  }
  ...
  function doSave(btn) {
    try {
      ...
      if (!st || !st.length) { flash(btn, 'no state'); return; }
      slot = st;
      try { localStorage.setItem(lsKey, toB64(st)); } catch (e) { /* quota / too big — keep in-memory only */ }
      flash(btn, 'saved');
  ...
  function doLoad(btn) {
    var st = slot;
    if (!st) { try { var b = localStorage.getItem(lsKey); if (b) st = fromB64(b); } catch (e) {} }
    if (!st) { flash(btn, 'no save'); return; }
    try { setStateBytes(st); flash(btn, 'loaded'); } catch (e) { console.error('[gx-state] load failed:', e); flash(btn, 'failed'); }
    refocus();
  }
