...
### Added
- **Atari 2600 CRT bezel** wired into `emulators/stella/play.html` — `_shared/bezels/70s.png`, the HMV "His Master's Voice / Ambassador 26" wood-cabinet TV (1920×1080, edge-to-edge with a transparent 4:3 screen hole at `23.75%/13.9%/52.4%×69.8%`). **First bezel on an EmulatorJS bundle** — EJS owns the canvas, so the screen hole holds `#game` (the EJS player) and EJS draws into it. EJS fits the canvas to the hole preserving aspect, leaving a top/bottom letterbox; closed it with `#game canvas { height: 100% !important; transform: translateX(-20px) }` (height-only stretch + a small horizontal nudge to centre the picture in the glass). Sets the pattern for eventually bezeling the rest of the EJS family. `70s.png` moves staging → wired (now six wired, four staging).
