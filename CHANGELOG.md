...
- **On-screen overlay sweep** — audited every emulator (visual capture of all 24 bundles) for stray chrome. **Javatari** drew a top toolbar (power / logo / fullscreen / settings) plus its console-switch faceplate — hid the toolbar (`#jt-bar`) but **kept the console panel**: its GAME RESET / GAME SELECT switches are the 2600's functional control surface and their keyboard equivalents are F11/F12, which the browser reserves (fullscreen / devtools), so the clickable switches are the only reliable way to use them. **JtyOne** (ZX81) showed a live `FPS: nn` readout — hidden (`#emulator_status`). Everything else already renders clean: EJS menu bars stay hidden, js7800/apple2/xroar/jsbeeb strip their upstream chrome, the bezel bundles frame the screen, and the Vectrex screen overlay + jsspeccy/intellivision start gates are intentional (not chrome).
- **Javatari gamedocs — corrected console-switch controls** — the 11 pages said Select = `F4`, Reset = `F5`; the actual javatari mapping (confirmed against its own on-screen panel) is GAME SELECT = F11, GAME RESET = F12 (browser-reserved), with difficulty on `F4`/`F9`. Since F11/F12 can't be used, rewrote the docs to direct players to **click GAME SELECT / GAME RESET** on the console panel (difficulty stays on the working `F4`/`F9` keys).
