...
- **On-screen overlay sweep** — audited every emulator (visual capture of all 24 bundles) for stray chrome. Two were leaking UI and are now stripped: **Javatari** drew its full console-switch panel (POWER / TV-TYPE / SELECT / RESET) plus a top toolbar — hidden via CSS (`#jt-bar`, `#jt-console-panel`; zeroed the panel's reserved `margin-bottom`). **JtyOne** (ZX81) showed a live `FPS: nn` readout — hidden (`#emulator_status`). Everything else already renders clean: EJS menu bars stay hidden, js7800/apple2/xroar/jsbeeb strip their upstream chrome, the bezel bundles frame the screen, and the Vectrex screen overlay + jsspeccy/intellivision start gates are intentional (not chrome).
