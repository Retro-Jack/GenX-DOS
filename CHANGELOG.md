...
### Changed
- Award BIOS POST hardware specs reconciled with the AMIBIOS Configuration table on the next phase: CPU now reads `80486SX CPU at 25MHz` (was `PENTIUM-S CPU at 75MHz`), `Memory Test : 8064K OK` (was `32768K`, now matches the 640 KB base + 7424 KB ext from the table), copyright years rolled back to 1991, and the BIOS ID footer date is `12/12/91` (matches the table's AMIBIOS Date).
