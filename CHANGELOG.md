...
- `emulators/jsbeeb/dist/discs/`: Bundled 15 BBC Micro disc images locally (~3.2 MB) so jsbeeb no longer fetches from `stairwaytohell.com` at runtime; `fs.js` URLs use empty-schema `disc1=<path>` (relative to dist/) instead of `disc1=sth:<path>`. Six STH paths corrected during the move: ChuckieEgg `AF/`→`AnF/`, Zalaga `Acornsoft/`→`Aardvark/`, Snapper `Acornsoft/Snapper.zip`→`Acornsoft/Snapper-v1-alt.zip`, ManicMiner `Tynesoft/`→`SoftwareProjects/`, CastleQuest `Superior/`→`Micropower/`, Uridium `Superior/`→`Hewson/`
