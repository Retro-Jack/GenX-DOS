...
- F1 key now opens an in-prompt help screen — content imported from the `Commands` wiki page (`github.com/Retro-Jack/DOS-Site/wiki/Commands`), reformatted as 80-col plain text. Sections: NAVIGATION, FILES, DISPLAY, STUB COMMANDS, KEYBOARD. Easter eggs deliberately omitted from the help. Implementation: `HELP_TEXT` array + `showFullHelp()` in `commands.js`; F1 (key code 112) handled in `keyboard.js` `doKeyDown` with `preventDefault` so the browser's own help doesn't open.
