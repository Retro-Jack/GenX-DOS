...
- American Megatrends pyramid logo (`prompt/img/ami-logo.png`, 409×136 PNG with transparent background — alpha-keyed by max-channel threshold so the original near-black background drops out while the red pyramid and white text stay opaque) shown left-aligned at the top of the boot animation. Replaces the prior Award POST text with a replicated AMIBIOS POST output: `www.ami.com` in red below the logo, then white POST lines covering AMIBIOS copyright, motherboard model (G31D-M7), CPU details (Pentium Dual-Core E5200 @ 2.50 GHz), memory, key-press hints (DEL/F9/F12), USB controller init, RAM total, USB devices, SATA auto-detect (ST3500641AS), USB mass storage scan, and a S.M.A.R.T. warning footer. Authentic enough to look like a real 2008 mid-range PC POST. Boot helper `renderAwardPost` renamed to `renderAmiBiosPost`; `bootR` colour helper added (red, palette index 4 = `#aa0000`, sourced from `img/f12.4.png`).
