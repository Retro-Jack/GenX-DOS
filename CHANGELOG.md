...
- **Audit doc sweep (10/06/2026)** — the PET migration pushed the engine count 19 → 20 (pet2001 is now a standalone engine, no longer riding EmulatorJS). Corrected the lagging "19 emulators" / "5 VICE cores" / "nine bundles share EJS" claims across `README.md`, `Project-Overview`, `Home`, `Roadmap`, `Emulators`, `Emulator-VICE-family`, and `File-Structure`. Untracked staged bezel `_shared/bezels/Pet.png` committed to complete the set. Refreshed the ATTRIBUTION bezel section: 10 bezels now, `PC.png` + `Acorn.png` wired (the rest staging), `Pet.png` row added, and a note that the two live bezels shipped before being source-cleared.
