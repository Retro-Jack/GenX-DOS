...
### Added
- **Amstrad CPC CRT bezel** wired into `emulators/amstradcpc/play.html` — the licence-cleared `_shared/bezels/Amstrad.png` (CPC464 + CTM640 monitor, 1502×992). Standard three-layer order (black backdrop → tiny8bit canvas at the detected screen cutout left 28.5%/top 14.4%/39%×44.4% → bezel PNG on top), `#loading` overlay repositioned onto the screen, 70s wallpaper behind, height capped at 100dvh−120px for breathing room. Carries a real CC-BY-SA 2.5 attribution comment (Bill Bertram) since this bezel is properly source-cleared. Fourth bezel to go live.
  - **Overlay cleanup:** floooh's WASM unconditionally draws a sokol-debugtext status bar (JOYSTICK/DISC/MOTOR/TRACK + frame timing) at the bottom of the canvas and a suspended-audio muted-speaker icon — neither is toggleable without rebuilding the binary. Masked the status bar with a black strip over the bottom of the (aspect-locked) cutout, and added the jsbeeb/Electron audio auto-unlock burst so the muted icon clears on load.
