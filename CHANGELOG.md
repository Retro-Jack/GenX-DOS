...
- Added a single-page boot animation: Award BIOS POST → cleared screen → extended-ASCII AMIBIOS System Configuration table (80-col double-rule outer border, single-rule column divider, two-column hardware fields styled after a 486-era POST screen) → DOS prompt menu. Each phase runs for 4 s. The animation lives entirely in `prompt/index.html` (boot helpers in `prompt/javascript/init.js`, full-bleed `#screen` div revealed during the animation, then hidden in favour of the `#prompt-wrap` for the terminal phase). Site root `index.html` is now a thin instant redirect to `prompt/`, so the AMIBIOS → menu transition happens with no page navigation, no `cls` flash, and no extra wait. Redundant `cls` removed from `c:\autoexec.bat` (menu.bat already clears).
