...
The newest fight was input rather than output. **USB gamepads** come free with the libretro cores, but the hand-built engines — the CPC, the Electron, the Spectrum, the ZX81 — predate any notion of a Gamepad API. The answer is a small shared shim that polls the pad and forges the keyboard events each engine already believes in, driven by a per-game map so the pad presses that game's own verified keys. Simple in theory; every engine found its own way to be difficult. JSSpeccy quietly re-binds its key listeners onto a div it injects at boot, so synthetic events aimed at the document sailed straight past it. The ZX81 is a machine where every key is live BASIC — Space is BREAK — so the game pages that need it filter out any key without an in-game function. And 3D Monster Maze's menus told players to press STOP and CONT, keywords printed on a 1981 keycap they don't have, so the tape image itself was patched byte-for-byte to name the real keys — and its two NEW tokens swapped for RUN, so no keypress can ever strand a player at a dead BASIC prompt.
...
What elevates GenX-DOS above "a pile of emulators" is that it has *opinions*. The scope isn't "8-bit era" — it's **8-bit feel**. The aesthetic decides, not the calendar: the Game Boy, Game Gear and Lynx are in, late-90s release dates be damned, because they look and sound like 8-bit machines; the Genesis and Amiga are out, because they don't. Each system's menu lists ten games, sorted by year — the Acorn Electron the lone exception, carrying the six snapshots its emulator (ElkJS) ships — with the command-line machines carrying a deliberate eleventh slot that boots a clean BASIC prompt. When a franchise qualifies, the *first* entry gets the nod, not the sequel. Given a choice of formats, the floppy release beats the CD.

The per-game documentation got the same treatment. A site-wide audit re-checked every control scheme against original manuals and disassemblies, and found that nearly every hand-built emulator's docs had *guessed* keys that turned out wrong — ZX Spectrum's per-publisher control quirks, a TI-99 "Hunt the Wumpus" that turned out to be the graphical joystick cart, not the text game, an Amstrad "Roland on the Run" page that described an entirely different game, a Spectrum Chuckie Egg whose documented keys belonged to some other platform's version and had to be re-established in-game, key by key. The corrections were sourced, not invented. It's the kind of pedantry that separates a toy from a reference.
