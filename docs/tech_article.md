...
The newest fight was input rather than output. **USB gamepads** come free with the libretro cores, but the hand-built engines — the CPC, the Electron, the Spectrum, the ZX81 — predate any notion of a Gamepad API. The answer is a small shared shim that polls the pad and forges the keyboard events each engine already believes in, driven by a per-game map so the pad presses that game's own verified keys, Start button included. Simple in theory; every engine found its own way to be difficult. JSSpeccy quietly re-binds its key listeners onto a div it injects at boot, so synthetic events aimed at the document sailed straight past it. The ZX81 is a machine where every key is live BASIC — Space is BREAK — so its game pages filter out any key without an in-game function. And 3D Monster Maze's menus told players to press STOP and CONT, keywords printed on a 1981 keycap they don't have, so the tape image itself was patched byte-for-byte to name the real keys — and its two NEW tokens swapped for RUN, so no keypress can ever strand a player at a dead BASIC prompt.
