...
│   ├── build-arcade-controls.py  reads the arcade core's own control data
│   │                        (controls.c + metadata XML, pinned) into arcade-controls.json
│   ├── build-arcade-gamedoc.py  rebuilds or checks the 100 arcade gamedocs from
│   │                        that data; arcade-labels.py holds the label corrections,
...
│   ├── genx.css             the @font-face set, the :root colour tokens
│   │                        and the site-wide link rule
│   ├── genx-doc.css         the document design language — every controls
│   │                        page, gamedoc, wiki page and the article
│   ├── wiki.css             the wiki frame only: sidebar, top bar, search
...
│
└── systems/
    ├── _shared/             shared JS + CSS + assets for every gamedoc + controls.html (474 pages) + play.html
...
| `docs/wiki/Search.html` | Generated by `build.py`, not written by hand. Full-text search over every section of every page, with the index inlined into the page rather than fetched — so it needs no `connect-src`, and works from a downloaded copy with no server. It is the **one** page in this wiki that runs JavaScript: its policy is the standard one plus `script-src 'sha256-…'`, the hash of exactly the script it carries, computed as that script is written so the two cannot drift. Every other page keeps `default-src 'none'` with no `script-src` at all |
