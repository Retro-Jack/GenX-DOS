...
check-doc-counts.sh       checks the counts quoted in the docs (and on the social card)
                          still match the tree, and that no doc names a renamed file
tools/                    developer tooling — not part of the shipping site
  ...
  build-arcade-controls.py  reads the arcade core's own control data into arcade-controls.json
  build-arcade-gamedoc.py   rebuilds (or checks) the 100 arcade gamedocs from that data;
  arcade-labels.py            label corrections, and arcade-gamedoc-overrides.json the pages
                              whose controls were set by hand, each with its reason
  gamefaqs-lookup.js      console helper that pulls scoring tables out of GameFAQs guides
  social-card.py          draws docs/images/genx-social.png's menu rows and stats line in
                          the prompt's own CP437 font, with the counts measured from the tree
...
systems/                  game counts below describe the live site; see ROMS.txt
  ...
  dos/                    IBM PC              — js-dos `emulators` layer + DOSBox WASM; 10 DOS games + MS-DOS 4.00
  ...
  trs80/                  Tandy TRS-80 Model III — sdltrs built from source to WASM + 10 .cmd
                          games and two bootable system disks (TRSDOS 1.3, LDOS 5.3.1)
...
The **Tandy CoCo** is the exception: XRoar has its own joystick support, so a pad is handed to the emulator itself rather than translated into key presses — `games.json` names the profile and the port per game. Only the games that want a joystick get one, because XRoar's keyboard joystick *consumes* the cursor keys and would otherwise break a keyboard-controlled game.

The **Vectrex** reads a pad natively too, and is the one machine here where that matters beyond convenience. Its controller was genuinely analogue in 1982, and the engine drives those same axes from the **left stick** — how far you push it counts, not just which way, which no D-pad reproduces. Every game reads the stick and the four face buttons, so nothing is per-game about it.

**A pad needs the browser to be able to enumerate it.** Chromium browsers only reveal a gamepad to a page that currently has focus, so if a pad is not picked up, click the game area and press a button on it again. That is the whole of it on the CoCo and Vectrex pages, which drive a real joystick rather than synthesizing key events the way the shim bundles do. A sandboxed browser is worth ruling out before anything else: a Flatpak or Snap build that cannot see the host's `/run/udev` enumerates no gamepads at all, on any site, because that is how a browser finds them on Linux.
...
**If you hold rights in anything bundled here — or legally represent someone who does — and you raise an identified copyright or IP concern, it will be removed immediately.** A request needs three things: who you are, your standing (rights holder, or who you represent and in what capacity), and what the material is with the basis of the concern. No notice format, registration certificate or legal citation is required — that is simply so nothing is pulled on an anonymous say-so. Email admin@genx-dos.fun — no account needed anywhere — or open an issue at <https://github.com/Retro-Jack/GenX-DOS/issues> if you'd rather it were public. Applies to everything here, not just games: titles, ROMs and BIOS images, emulator code, artwork, fonts, docs, names and marks. Full statement in [ATTRIBUTION.md](ATTRIBUTION.md#removal-upon-request).
