...
**A pad on the CoCo pages needs Firefox.** Chromium browsers only reveal a gamepad to a page that currently has focus, and XRoar asks for the joystick list once at startup and never asks again — so the pad arrives after the only question was put. Firefox has no such rule and the same pad works there at once. This is not particular to this site: XRoar's own online build behaves identically. Nothing the page can do about it, so `systems/_shared/genx-gamepad-browser-notice.js` says so on screen rather than leaving the player guessing.
