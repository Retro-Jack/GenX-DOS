...
**A pad on the CoCo and Vectrex pages needs Firefox.** Chromium browsers only reveal a gamepad to a page that currently has focus, and neither engine asks again after startup — XRoar reads the joystick list once, and jsvecx never binds one at all — so the pad arrives after the only question was put. Firefox has no such rule and the same pad works there at once. This is not particular to this site: XRoar's own online build behaves identically. Nothing the page can do about it, so `systems/_shared/genx-gamepad-browser-notice.js` says so on screen rather than leaving the player guessing, and points at the bundle's Controls page for the keyboard equivalents. The shim bundles are unaffected: they synthesize key events, which Chromium delivers once the page has been clicked.
